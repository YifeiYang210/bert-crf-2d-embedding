...
from utils import NerProcessor, get_labels, convert_examples_to_features, get_Dataset
...
def evaluate(args, data, model, id2label, all_ori_tokens):
    ...
    for b_i, (input_ids, input_mask, segment_ids, label_ids, bbox, bbox_pos_id, bbox_num) in enumerate(tqdm(dataloader, desc="Evaluating")):
        ...
        with torch.no_grad():
            logits = model.predict(input_ids, segment_ids, input_mask, bbox, bbox_pos_id, bbox_num)
...
def main():
    ...
    if args.do_train:
        ...
        train_examples, train_features, train_data = get_Dataset(args, processor, tokenizer, label_list, mode="train")
        train_sampler = RandomSampler(train_data)
        train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size)

        if args.do_eval:
            eval_examples, eval_features, eval_data = get_Dataset(args, processor, tokenizer, label_list, mode="eval")
        ...
        for ep in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                outputs = model(input_ids, bbox, bbox_pos_id, bbox_num, label_ids, segment_ids, input_mask)
                loss = outputs
    ...
    if args.do_test:
        ...
        test_examples, test_features, test_data = get_Dataset(args, processor, tokenizer,label_list, mode="test")
        ...
        for b_i, (input_ids, input_mask, segment_ids, label_ids, bbox, bbox_pos_id, bbox_num) in enumerate(tqdm(test_dataloader, desc="Predicting")):
